...
@Component({selector:'licenses-workspace',imports:[FormsModule,JsonPipe,FleetOperationResult,MetricBarChart],template:`
<div class="toolbar">
...
<section>
  <div class="section-title"><h2>Usage by user</h2><label>Filter user<input [(ngModel)]="userFilter" placeholder="User ID"></label></div>
  ...
  @for(item of users(); track item.instanceId + item.row.userId){
...
<section>
  <div class="section-title"><h2>Usage by process</h2><label>Filter type<select [(ngModel)]="typeFilter"><option value="">All types</option><option>User</option><option>CSP</option><option>Mixed</option><option>Grace</option></select></label></div>
  ...
  @for(item of processes(); track item.instanceId + item.row.pid){
