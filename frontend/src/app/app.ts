...
@Component({selector:'app-root',imports:[FormsModule,JsonPipe,InstanceSelector,FleetOperationResult,ResourceWorkspace,SecurityWorkspace,FleetQuery,SystemWorkspace,EventsWorkspace,OverviewWorkspace,LicensesWorkspace,MetricBarChart,VectorWorkspace,Pagination],templateUrl:'./app.html'})
export class App {
  ...
  processPage=signal(1);readonly processPageSize=20;
  ...
  async search(){if(!this.selected().length)return;this.processPage.set(1);this.busy.set(true);this.error.set('');try{this.processes.set(await api<FleetResult<ProcessRow[]>>('/processes?instances='+this.selected().join(',')+'&filter='+encodeURIComponent(this.filter)));}catch(e){this.fail(e);}finally{this.busy.set(false);}}
  ...
  visibleProcessRows(){const start=(this.processPage()-1)*this.processPageSize;return this.processRows().slice(start,start+this.processPageSize);}
