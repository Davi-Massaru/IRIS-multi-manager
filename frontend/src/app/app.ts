...
@Component({selector:'app-root',imports:[FormsModule,JsonPipe,InstanceSelector,FleetOperationResult,ResourceWorkspace,SecurityWorkspace,FleetQuery,SystemWorkspace,EventsWorkspace],templateUrl:'./app.html'})
export class App {
  ...
  username='_SYSTEM';password='';filter='';
  ...
  async connect(){this.busy.set(true);this.error.set('');try{for(const id of this.selected()){try{await api(`/instances/${id}/connect`,{username:this.username,password:this.password});}catch(e){this.fail(e);}}await this.refreshStatus();}finally{this.password='';this.busy.set(false);}}
